...
| Fail-closed | 72 actions: 61 that this adapter cannot express, and the 11 that the reference adapter does not support either. Each must raise an error whose message [`conformance-ledger.json`](conformance-ledger.json) pins ([ADR 0010](../docs/adr/0010-each-adapter-owns-its-conformance-ledger.md)), so a typo or a transport error cannot pass as the refusal |
