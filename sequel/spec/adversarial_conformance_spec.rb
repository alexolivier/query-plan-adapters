...
# guard.
#
# This file contains only the configuration for Sequel: the schema is in
# spec/support/adversarial_models.rb, and the attribute map is in spec/support/corpus_attributes.rb.
...
RSpec.describe "adversarial conformance" do
  ...
  describe "corpus" do
    ...
    it "seeds the to-one chain that the corpus describes" do
      ...
      stored = Database::DB[:adversarial_resources]
        .left_join(:adversarial_parents, {resource_id: :id}, table_alias: :p)
        .left_join(:adversarial_inners, {parent_id: Sequel[:p][:id]}, table_alias: :i)
        # Aliased, because the two hops name the same column and a row hash keeps one of them.
        .select_map([
          Sequel[:adversarial_resources][:id],
          Sequel[:p][:a_string].as(:parent_string),
          Sequel[:i][:a_string].as(:inner_string)
        ])
    ...
    it "seeds the number and boolean lists in corpus order, null elements included" do
      {
        ...
        model.order(:resource_id, :position).select_map([:resource_id, :value]).each do |id, value|
          stored[id] << value
        end
  ...
  describe "null attribute representation" do
    ...
    it "lets the declaration of an attribute override the convention of the call" do
      dataset = Cerbos::Sequel.query_plan_to_dataset(
        plan: AdversarialOracle.plan("null-eq"),
        model: AdvResource,
        attributes: ATTRIBUTES,
        null_attribute_representation: :omitted
      )
      expect(dataset.select_map(:id).sort).to eq(AdversarialOracle.allowed_ids("null-eq"))
      ...
      }.to raise_error(Cerbos::Sequel::Error, /null constant/)
    end
