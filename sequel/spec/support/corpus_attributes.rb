...
module CorpusAttributes
  ...
  ATTRIBUTES = {
    ...
    # action that reads them indexes them, and an association mapping has no element order, so the
    ...
    # association, and `subCategories`/`subNames` are nested associations in its fields. Each seed
    ...
    #
    # The nesting is deliberate. A flat `many_to_many` through the categories table gives
